...
train_bart <- function(x, y, num_trees = 1, # number of trees
                       ...
                       hyper = list(alpha = 0.95, beta = 2, c = 0.01, alpha_sig=5, beta_sig=5), # Give values to the hyperparameters
...
sim_res <- train_bart(x = sim_x, y = sim_y, mcmc_par = list(iter = 3000, burn = 0, thin = 2),hyper = list(alpha = 0.95, beta = 2, c = 0.01, alpha_sig = 10, beta_sig = 4), control = list(node_min_size = 100),init = list(sigma = 0.01))
sim_res$tree[[10]]$info[which(sim_res$tree[[10]]$info[,"terminal"]==1),"mu"]*sim_res$scale_y + sim_res$center_y
```
...
y.tr <- sim_y[tr.index]
y.va <- sim_y[-tr.index]
cv.bart <- train_bart(x.tr, y.tr, mcmc_par = list(iter = 1000, burn = 2000, thin = 2),hyper = list(alpha = 0.95, beta = 2, c = 0.1, alpha_sig = 10, beta_sig = 4, control = list(node_min_size = 100)))
